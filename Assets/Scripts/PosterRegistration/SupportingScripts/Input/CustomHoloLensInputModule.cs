...
namespace PosterAlignment.InputUtilities
{
    ...
    public class CustomHoloLensInputModule : PointerInputModule
    {
        ...
        private bool EnsureModules()
        {
            ...
            return (this.mouseManager != null && this.gestureManager != null);
        }
        ...
        private bool SendSubmitCancelEvent()
        {
            ...
            bool doSubmit = this.mouseManager.ShouldSubmit(); // should be false, but may want to support multi-button type mouse
            ...
            bool doCancel = this.mouseManager.ShouldCancel();
            if (!doCancel)
            {
        ...
        private bool SendMoveEvent()
        {
            ...
            bool doMove = this.mouseManager.ShouldMove(out movement);
            if (!doMove)
            {
        ...
        {
            var pressed =  this.mouseManager.IsPressed(button) || this.gestureManager.IsPressed(button);
            var released = this.mouseManager.IsReleased(button) || this.gestureManager.IsReleased(button);
