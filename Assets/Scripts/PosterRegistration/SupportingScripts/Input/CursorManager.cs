...
namespace PosterAlignment.InputUtilities
{
    public class CursorManager : ImprovedSingletonBehavior<CursorManager>
    {
        ...
        private void OnClick(GameObject hitObject)
        {
            ...
            {
                if (hitObject.name == "InstructionText" || hitObject.name == "Poster")
                {
                    if ((ZoneManager.currentZone == null) || (!ZoneManager.currentZone.IsCalibrating))
                    {
                        ZoneManager.AlignZone(0);
                    }

                }
                else if (hitObject.name == "Stop Calibration")
                {
